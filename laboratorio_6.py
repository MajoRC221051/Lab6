...
Original file is located at
    https://colab.research.google.com/drive/1z7sAqgb70B-UW9s5Tj0XNqsGkS95EgvD
...
if tweets:
    ...
    print("⚠️ No se pudieron cargar tweets válidos")
    df = pd.DataFrame()
...
# --- Distribución por hora del día (para detectar horarios de atascos) ---
...
try:
    ...
    if 'clean_text' in df.columns:
        ...
        print("⚠️ Usando rawContent para nube de palabras (clean_text no encontrado)")
    ...
    print("\n⚠️ Nube de palabras omitida. Instala 'wordcloud' con: pip install wordcloud")
except Exception as e:
    print(f"\n⚠️ Error en nube de palabras: {e}")
...
print(f"2) %Retweets={porc_retweets:.1f}%, %Respuestas={porc_respuestas:.1f}% -> "
      f"tendencia más a {'amplificar (RT)' if porc_retweets>porc_respuestas else 'conversar (replies)'}.")

# P3: ¿Quiénes son los actores más citados por la cuenta/usuarios?
print(f"3) Usuarios más mencionados en el conjunto: {[f'@{u[0]}' for u in top_mencionados[:5]]}")

# P4: Información adicional útil
print(f"4) Total de hashtags únicos: {len(hashtag_counter):,}")
print(f"5) Total de usuarios únicos mencionados: {len(mencionados):,}")
...
Los picos de interacción se registran principalmente a las **0:00 (404 tweets), 19:00 (388 tweets) y 20:00 (387 tweets)**. Esto muestra que la conversación en torno a @traficoGT se activa fuertemente en la **madrugada y en la noche**, coincidiendo con momentos de tráfico intenso y mayor participación ciudadana en temas políticos o sociales.
...
if H.number_of_nodes() > MAX_NODES_TO_DRAW:
    # Toma el componente débilmente conectado más grande y submuestrea por grado
...
print(f"Número de componentes débilmente conectados: {len(componentes_debiles)}")
print(f"Número de componentes fuertemente conectados: {len(componentes_fuertes)}")

# Tamaño del componente gigante
if componentes_debiles:
    tamaño_giante = len(max(componentes_debiles, key=len))
    print(f"Tamaño del componente gigante: {tamaño_giante} nodos")
    print(f"Porcentaje de nodos en componente gigante: {(tamaño_giante/G.number_of_nodes())*100:.1f}%")
...
# Diámetro: usar componente gigante y convertir a no dirigido (si no es conexa)
if G.number_of_nodes() > 1 and G.number_of_edges() > 0:
    ...
    if nx.is_connected(Gu):
        ...
        # Aproximación: diámetro del componente más grande (que ya es Gu) con excentricidades válidas
        # Si no es totalmente conexo (raro en Gu), tomar diámetro de la mayor componente conectada interna
...
# Coeficiente de agrupamiento (usar versión no dirigida para interpretación clásica)
...
import matplotlib.pyplot as plt
from collections import Counter
import community.community_louvain as cl # Importa el sub-módulo que contiene best_partition

# Asegúrate de que G esté definido. Descomenta la línea de abajo si no lo estás cargando de otra fuente.
...
# Caracterización: tamaño, interacciones internas, temas (hashtags) por comunidad
...
if 'user_normalized' in df.columns and 'hashtags' in df.columns:
    ...
    print("⚠️ Required columns 'user_normalized' or 'hashtags' not found in DataFrame 'df'.")
...
if not df_user_hashtags.empty:
    ...
    print("\n⚠️ Cannot characterize communities: df_user_hashtags is empty.")
...
# --- Visual: destacar los 20 nodos con mayor in-degree (posibles 'hubs' de recepción) ---
...
print("Tamaños de las primeras 10 componentes:", [s for s,_ in component_sizes[:10]])

# Identificar componentes pequeñas / aisladas (ej. tamaño <= 5)
small_thresh = 5
isolated_components = [c for c in components if len(c) <= small_thresh]
print(f"Componentes pequeñas (<= {small_thresh} nodos): {len(isolated_components)}")

# Analizar las componentes aisladas: topologías y proporciones de interacción interna vs externa
isolated_summary = []
...
# Mostrar ejemplos con contenido: para cada componente pequeña, listar usuarios y top hashtags/tópicos
...
small_thresh = 5
isolated_components = [c for c in components if len(c) <= small_thresh]
print(f"\nComponentes pequeñas (<= {small_thresh} nodos): {len(isolated_components)}")
...
from tqdm.notebook import tqdm # Importar al inicio de tu script o función
...
# df = pd.DataFrame({'rawContent': ["Me encanta este producto", "No me gusta para nada", "Es regular", "Excelente servicio", "Qué mal día"]} * 100)
...
* **Palabras clave:** shoooooooooooooo, prófugo, cc, jajaja, yoapoyoalpresidentearevalo, golpe, ojo, puros, show, hueeeeco.
...
* **En la red de @traficogt:** Es **altamente probable** que el **Tema 2 ("Tráfico y Vialidad")** sea el más recurrente y dominante. Esta cuenta se dedica explícitamente a informar sobre el tráfico, por lo que las discusiones sobre calles, zonas, avenidas y congestión serían su núcleo. Otros temas, como el Tema 0 ("Guatemala y los Guatemaltecos") podrían aparecer en menor medida, quizás cuando se relaciona el tráfico con el desarrollo urbano o problemas generales de la ciudad.
